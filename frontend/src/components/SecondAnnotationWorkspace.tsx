...
export default function SecondAnnotationWorkspace({ datasetId, onGoBack }: SecondAnnotationWorkspaceProps) {
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-surface-container-low text-on-surface">
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-outline-variant/40 bg-surface-container-lowest px-6">
        <div className="flex items-center gap-4">
          <button onClick={onGoBack} className="p-2 text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface" aria-label="Back">
            <ArrowLeft className="h-5 w-5" />
          </button>
          <div>
            <h1 className="text-sm font-semibold">{payload?.filename || datasetId}</h1>
            <p className="text-[11px] text-on-surface-variant">二次人工标注校验 · {payload?.active_version || 'loading'}</p>
          </div>
        </div>
      ...
      {(message || error) && (
        <div className={`border-b px-6 py-2 text-xs font-semibold ${error ? 'border-error/30 bg-error/10 text-error' : 'border-primary/20 bg-primary/10 text-primary'}`}>
          {error || message}
        </div>
      )}

      <div className="flex min-h-0 flex-1">
        <aside className="w-64 shrink-0 overflow-y-auto border-r border-outline-variant/40 bg-surface-container-lowest p-4">
          <h2 className="mb-3 text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">页面列表</h2>
          <div className="space-y-2">
            {pageOptions.map(({ page, index }) => (
              <button
                ...
                className={`flex w-full items-center gap-3 border p-2 text-left ${index === currentPageIndex ? 'border-primary/40 bg-surface-container-high' : 'border-outline-variant/40 bg-surface-container hover:bg-surface-container-high'}`}
              >
                <div className="h-14 w-10 shrink-0 bg-surface-container-high bg-cover bg-center" style={{ backgroundImage: `url(${page.image_url})` }} />
                <div className="min-w-0">
                  <p className="text-xs font-semibold">Page {page.page_id + 1}</p>
                  <p className="text-[10px] text-on-surface-variant">{page.blocks.length} blocks</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <main className="relative flex min-w-0 flex-1 flex-col bg-surface-container">
          <div className="absolute left-4 top-4 z-20 flex items-center gap-2 border border-outline-variant/40 bg-surface-container-low/90 px-3 py-2 text-xs text-on-surface-variant">
          ...
          <div className="flex flex-1 items-center justify-center overflow-auto p-12">
            {currentPage ? (
              <div
                ...
                className="relative shrink-0 bg-surface-bright bg-cover bg-center shadow-2xl"
                style={{
                  width: canvasWidth,
                  height: canvasHeight,
              ...
              >
                {currentPage.blocks.map((block) => {
                  ...
                  return (
                    ...
                    >
                      <span className="absolute -top-5 left-0 bg-primary/80 px-1.5 py-0.5 text-[10px] text-on-primary">{block.level ? `${block.level} · ` : ''}{block.label}</span>
                      <button
                        type="button"
              ...
              <div className="text-on-surface-variant">正在加载标注数据...</div>
            )}
          </div>

          <div className="absolute bottom-6 left-1/2 flex -translate-x-1/2 items-center gap-2 border border-outline-variant/40 bg-surface-container-low/90 px-4 py-2">
            <button onClick={() => setScale((value) => Math.max(0.35, value - 0.1))} className="p-2 text-on-surface-variant hover:bg-surface-container-high"><ZoomOut className="h-4 w-4" /></button>
            <span className="w-12 text-center text-xs">{Math.round(scale * 100)}%</span>
            <button onClick={() => setScale((value) => Math.min(1.8, value + 0.1))} className="p-2 text-on-surface-variant hover:bg-surface-container-high"><ZoomIn className="h-4 w-4" /></button>
            <button onClick={() => setScale(0.78)} className="p-2 text-on-surface-variant hover:bg-surface-container-high"><Maximize2 className="h-4 w-4" /></button>
          </div>
        </main>

        <aside className="w-80 shrink-0 overflow-y-auto border-l border-outline-variant/40 bg-surface-container-lowest p-4">
          <h2 className="mb-3 text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">选框属性</h2>
          {selectedBlock ? (
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-on-surface-variant">Label</label>
                <select
                  value={selectedBlock.label}
                  onChange={(event) => updateSelectedBlock({ label: event.target.value, block_type: event.target.value }, ['label'])}
                  className="w-full border border-outline-variant/40 bg-surface-container px-3 py-2 text-sm text-on-surface outline-none focus:border-primary/40"
                >
                  {labelTypes.map((label) => <option key={label} value={label}>{label}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-on-surface-variant">Heading Level</label>
                <select
              ...
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-on-surface-variant">Text</label>
                <textarea
              ...
              <button onClick={deleteSelectedBlock} className="flex w-full items-center justify-center gap-2 rounded-[0.75rem] border border-error/30 bg-error/10 px-3 py-2 text-sm font-semibold text-error hover:bg-error/15">
                <Trash2 className="h-4 w-4" />
                删除选框
              </button>
            </div>
          ) : (
            <div className="rounded border border-outline-variant/40 bg-surface-container p-4 text-sm text-on-surface-variant">
              <Plus className="mb-2 h-5 w-5" />
              选择一个选框，或在画布空白处拖拽新增选框。
            </div>
          )}
          <div className="mt-6">
            <h3 className="mb-2 text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">当前页选框</h3>
            <div className="space-y-2">
              {currentPage?.blocks.map((block) => (
                <button
                  key={block.id}
                  onClick={() => setSelectedBlockId(block.id)}
                  className={`w-full border p-2 text-left text-xs ${block.id === selectedBlockId ? 'border-cyan-300/50 bg-cyan-300/10' : 'border-outline-variant/40 bg-surface-container hover:bg-surface-container-high'}`}
                >
                  <p className="font-semibold text-on-surface">{block.level ? `${block.level} · ` : ''}{block.label}</p>
                  <p className="truncate text-on-surface-variant">{block.text || block.id}</p>
                </button>
              ))}
            </div>
          </div>
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-3 py-2 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${
        primary ? 'rounded-[0.75rem] bg-primary text-on-primary hover:bg-primary/90' : danger ? 'rounded-[0.75rem] border border-error/30 text-error hover:bg-error/10' : 'rounded-[0.75rem] border border-outline-variant/40 text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface'
      }`}
    >
      {icon}
      {label}
